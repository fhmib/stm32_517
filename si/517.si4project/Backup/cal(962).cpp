...
#define FILE_HEADER_OFFSET2            0x107C
#define CAL_HEADER_OFFSET              0x10c0
#define CAL_HEADER2_OFFSET             0x10e0
...
void table_init_for_364()
{
  ...
  for (i = 0; i < (60 - 24); ++i) {
    i2c_eeprom_write_byte(eeprom_addr, EEPROM_HEADER_OFFSET + 24, 0);
  }
  ...
  // file header 1 0x1040 ~ 0x6F
  i2c_eeprom_write_buffer(eeprom_addr, FILE_HEADER_OFFSET1, file_header1, 16);
  for (i = 0; i < 24; ++i) {
    i2c_eeprom_write_byte(eeprom_addr, FILE_HEADER_OFFSET1 + 16, 0);
  }
  ...
  // file header 2 0x107C ~ 0xBB
  for (i = 0; i < 16 * 4; ++i) {
    i2c_eeprom_write_byte(eeprom_addr, FILE_HEADER_OFFSET2 + 24, 0);
  }

  // cal header 0x10C0 ~ 0x10DF
  i2c_eeprom_write_buffer(eeprom_addr, CAL_HEADER_OFFSET, cal_header, 32);
  write_byte_to_eeprom(CAL_HEADER_OFFSET + 32, 0, 12);

  // 0x10E0 ~ 0x113C
  i2c_eeprom_write_buffer(eeprom_addr, CAL_HEADER2_OFFSET, cal_header2_for_364, cal_header2_length_364);
  write_byte_to_eeprom(CAL_HEADER2_OFFSET + cal_header2_length_364, 0, (0x113C - 0x10E0 + 1) - cal_header2_length_364);
}

void write_byte_to_eeprom(uint32_t addr, unsigned char byte, uint32_t count)
{
  for (int i = 0; i < count; ++i) {
    i2c_eeprom_write_byte(eeprom_addr, addr + i, byte);
  }
}
