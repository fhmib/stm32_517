...
int32_t cal_il(int32_t argc, char **argv)
{
  ...
  if (!strcmp(argv[2], "dump")) {
    addr = pd_addr_array[num - 1];
    dump_cali(addr, pd_cali_count, DUMP_MODE_IL);
    return 0;
  }
...
int32_t cmd_cal(int32_t argc, char **argv)
{
  ...
  } else if (cil && strncmp(argv[1], "il", 2) == 0 && strlen(argv[1]) > 2) {
    return cal_pd(argc, argv);
  } else if (cvkb && strncmp(argv[1], "vkb", 2) == 0 && strlen(argv[1]) > 2) {
    return cal_pd(argc, argv);
  } else if (cvt && strncmp(argv[1], "vt", 2) == 0 && strlen(argv[1]) > 2) {
    return cal_pd(argc, argv);
  } else {
    Serial.println("Wrong arg");
    return -1;
